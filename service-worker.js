    ...
    event.waitUntil(
        caches.open(CACHE_NAME)
            .then(cache => {
                ...
                return cache.addAll(ASSETS.map(asset => new Request(asset, { mode: 'no-cors' })));
            })
            .then(() => {
        ...
        caches.match(event.request)
            .then(response => {
                if (response) {
                    ...
                    if (event.request.url.includes('.js') || event.request.url.includes('.css') || event.request.url.includes('.html')) {
                ...
                if (event.request.url.includes('.js') || event.request.url.includes('.css') || event.request.url.includes('.html')) {
                    console.log('⬇️ SW: Downloading ->', event.request.url.split('/').pop());
                }
                            ...
                            caches.open(CACHE_NAME)
                                .then(cache => {
                                    ...
                                    try {
                                        ...
                                        console.warn('⚠️ SW: No se pudo cachear:', event.request.url.split('/').pop());
                                    }
                                })
                                .catch(error => console.warn('⚠️ SW: Error al abrir caché:', error));
                        }
                        return response;
                    })
                    .catch(error => {
                        console.error('❌ SW: Error en fetch:', event.request.url.split('/').pop(), error.message);
